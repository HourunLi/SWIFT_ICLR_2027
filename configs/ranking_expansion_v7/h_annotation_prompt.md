...
这份文件只规定标注行为。实际 smoke/reserve 包生成后，以各自 package report
里的绝对路径和行数为准。
...
## Reserve：给标注者 A（GPT-5.6-sol xhigh）

在一个新的独立会话中，完整沿用上方“给标注者 A”的任务定义、判断规则、
JSON schema 和逐行校验要求，但只处理下面的 reserve 文件，不要重新处理 smoke：

```text
输入：
/prodcpfs/user/panzhixin/ICLR_2027/run_artifacts/ranking_expansion_v7/pre_annotation/packages/reserve/annotator_a/hallucination.jsonl

输出：
/prodcpfs/user/panzhixin/ICLR_2027/run_artifacts/ranking_expansion_v7/pre_annotation/annotation/reserve_a_gpt56sol.jsonl

不得读取 annotator_b、PRIVATE_package_index.jsonl、smoke 标签、checker/reference
answer、其他模型输出或历史标签。输入共 800 行；必须覆盖每个 item_id 一次且仅
一次，并在结束前验证 JSONL 可逐行解析、item_id 无重复且输入输出均为 800 行。
```

## Reserve：给标注者 B（Claude Opus 5 high）

在另一个新的独立会话中，完整沿用上方“给标注者 B”的任务定义、判断规则、
JSON schema 和逐行校验要求，但只处理下面的 reserve 文件，不要重新处理 smoke：

```text
输入：
/prodcpfs/user/panzhixin/ICLR_2027/run_artifacts/ranking_expansion_v7/pre_annotation/packages/reserve/annotator_b/hallucination.jsonl

输出：
/prodcpfs/user/panzhixin/ICLR_2027/run_artifacts/ranking_expansion_v7/pre_annotation/annotation/reserve_b_claude_opus5.jsonl

不得读取 annotator_a、PRIVATE_package_index.jsonl、smoke 标签、checker/reference
answer、其他模型输出或历史标签。输入共 800 行；必须覆盖每个 item_id 一次且仅
一次，并在结束前验证 JSONL 可逐行解析、item_id 无重复且输入输出均为 800 行。
```
