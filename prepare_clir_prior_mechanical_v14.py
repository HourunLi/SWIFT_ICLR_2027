#!/usr/bin/env python
"""Prepare, verify, and evaluate the fresh CLIR Prior mechanical-v14 smoke."""
...
DEFAULT_PROTOCOL = PROJECT_ROOT / "configs/data_expansion_prior_v14/protocol.json"
DEFAULT_OUTPUT = PROJECT_ROOT / "run_artifacts/data_expansion_prior_v14/pre_annotation"
DEFAULT_PROMPT = PROJECT_ROOT / "configs/data_expansion_prior_v14/annotation_prompt.md"
DEFAULT_LAUNCH_A = PROJECT_ROOT / "configs/data_expansion_prior_v14/launch_prompt_a.txt"
DEFAULT_LAUNCH_B = PROJECT_ROOT / "configs/data_expansion_prior_v14/launch_prompt_b.txt"
EDGE_SOURCE = PROJECT_ROOT / "src/clir_prior_edge_candidates_v14.py"
SMOKE_SOURCE = PROJECT_ROOT / "src/clir_prior_mechanical_smoke_v14.py"
...
def _load_protocol(path: Path) -> dict[str, Any]:
    ...
    if protocol.get("schema_version") != "clir-prior-mechanical-recall-smoke-v14":
        raise ValueError("unsupported Prior v14 protocol schema")
    return protocol
...
def _verify_parent_files(protocol: Mapping[str, Any]) -> dict[str, str]:
    ...
    for name in names:
        ...
        if actual != expected:
            raise ValueError(f"Prior v14 parent hash drift: {name}")
    ...
    status_bindings = (
        ...
        ("v14_dev_report_path", "v14_dev_status"),
    )
    for path_key, status_key in status_bindings:
        report = json.loads(
            _project_path(parent[path_key]).read_text(encoding="utf-8")
        )
        if report.get("status") != parent[status_key]:
            raise ValueError(f"Prior v14 parent status drift: {status_key}")
    return bindings
...
def _recompute(
    ...
    v13_proposals = read_jsonl(_project_path(parent["v13_proposals_path"]))
    exclusions = [*v12_proposals, *v13_proposals]
    excluded_queries = {str(row["query_id"]) for row in exclusions}
    excluded_clusters = {str(row["cluster_id"]) for row in exclusions}
    proposals, selection = select_fresh_natural_rows_v14(
        all_rows,
...
def _package_path(root: Path, annotator: str, shard_index: int) -> Path:
    return (
        root
        / f"packages/annotator_{annotator}/prior_v14_{annotator}_{shard_index:02d}.jsonl"
    )


def _label_path(root: Path, annotator: str, shard_index: int) -> Path:
    return root / f"labels_{annotator}/prior_v14_{annotator}_{shard_index:02d}.jsonl"
...
def _code_bindings() -> dict[str, str]:
    return {
        "edge_candidate_source_file_sha256": file_sha256(EDGE_SOURCE),
        "v14_smoke_source_file_sha256": file_sha256(SMOKE_SOURCE),
...
def command_prepare(args: argparse.Namespace) -> None:
    if _git_dirty():
        raise RuntimeError("Prior v14 package freeze requires a clean Git commit")
    protocol_path = Path(args.protocol).resolve()
    root = Path(args.output).resolve()
    if root.exists() and any(root.rglob("*")):
        raise FileExistsError(f"Prior v14 output is not empty: {root}")
    protocol = _load_protocol(protocol_path)
    parent_bindings = _verify_parent_files(protocol)
    proposals, selection, packages, private, construction, burden = _recompute(
        protocol
    )
    ...
    common_metadata = {
        ...
        "v13_terminal_decision_unchanged": True,
        **parent_bindings,
        **prompt_bindings,
        **code_bindings,
    }
    proposal_path = root / "proposals/prior_mechanical_recall_natural_48.jsonl"
    ...
    report = {
        "schema_version": "clir-prior-mechanical-recall-package-report-v14",
        "status": "PASS_PRIOR_V14_FRESH_BLIND_PACKAGES_READY",
        "protocol_file_sha256": file_sha256(protocol_path),
        "code_commit": _git_head(),
        "code_dirty": False,
...
def command_verify(args: argparse.Namespace) -> None:
    ...
    shard_count = int(protocol["annotation"]["shards_per_annotator"])
    verification = {
        "schema_version": "clir-prior-mechanical-recall-package-verification-v14",
        "status": (
            "PASS_PRIOR_V14_PACKAGE_INDEPENDENT_RECOMPUTE"
            if not mismatches
            else "FAIL_PRIOR_V14_PACKAGE_INDEPENDENT_RECOMPUTE"
        ),
    ...
    if verification_path.exists():
        ...
        if old != verification:
            raise ValueError("Prior v14 package verification drift")
    else:
...
def command_evaluate(args: argparse.Namespace) -> None:
    ...
    if verification.get("status") != "PASS_PRIOR_V14_PACKAGE_INDEPENDENT_RECOMPUTE":
        raise ValueError("Prior v14 package verification is not PASS")
    package_report_path = root / "packages/package_report.json"
    if verification.get("package_report_file_sha256") != file_sha256(
        package_report_path
    ):
        raise ValueError("Prior v14 package report binding drift")
    ...
    for annotator in ("a", "b"):
        ...
        if actual_paths != expected_paths:
            raise ValueError(
                f"Prior v14 labels_{annotator} population mismatch: "
        ...
        for shard_index in range(shard_count):
            ...
            if len(package_rows) != rows_per_shard or len(label_rows) != rows_per_shard:
                raise ValueError(
                    f"Prior v14 shard {key} must contain {rows_per_shard} rows"
                )
            ...
            if len(label_ids) != len(set(label_ids)) or set(label_ids) != set(
                ...
                raise ValueError(f"Prior v14 label/package ID mismatch: {key}")
    ...
    if evaluation_path.exists():
        ...
        if old != report:
            raise ValueError(
                "Prior v14 evaluation report already exists with different content"
            )
    else:
