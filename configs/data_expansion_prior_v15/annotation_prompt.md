# CLIR Prior v15：只判断主链 block，Key/Complete 由程序生成

你只标当前公开 JSONL。机器已经把候选推理切成带编号的 `block`。你不需要标依赖边，也不要输出
...
- `Complete` = 你标成 `main_step` 的所有 block；
- `Key` = `final_block_id` 对应 block 的全部原始 unit；
- 路径即使有错，Key 仍是“最终完成该候选答案的结构步骤”，不是最早错误；
- 最早错误完全属于 Hallucination 模块，本轮不要定位它。

因此你必须按候选实际写出的推理判断主链，不能改写成自己更短的解法。
...
- `plan_or_heading`：只说“接下来计算……”或标题，没有产生可用内容。
- `formula_only`：只写通用公式，没有代入本题对象、数值或约束。
- `duplicate`：完整重复前面已经表达的同一个结果。
- `answer_wrapper`：只包装/重复答案，没有新计算或选择。
- `unused_branch`：做了计算或推导，但候选最终答案没有使用它。

特别注意：`Let x=...`、`设 D=...`、把题意写成方程、以及连续代数改写，只要后面实际使用，都是
`main_step`，不能因为它像“定义”或“复述”就标成 `premise_restatement`。

## 3. Path 与最终 block

- 有可审计推理：`eligibility="usable"`；只有答案/拒答用 `no_auditable_reasoning`；切分后仍无法辨认
