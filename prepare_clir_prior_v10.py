#!/usr/bin/env python
"""Prepare, verify, and evaluate the canonical CLIR Prior smoke v10."""
...
DEFAULT_PROTOCOL = PROJECT_ROOT / "configs/data_expansion_prior_v10/protocol.json"
DEFAULT_OUTPUT_ROOT = (
    PROJECT_ROOT / "run_artifacts/data_expansion_prior_v10/pre_annotation"
)
...
def _load_protocol(path: Path) -> dict[str, Any]:
    ...
    if protocol.get("schema_version") != "clir-prior-canonical-smoke-v10":
        raise ValueError("unsupported Prior v10 protocol schema")
    if protocol.get("status") != "FROZEN_PRE_ANNOTATION_PREPARATION":
        raise ValueError("Prior v10 protocol is not frozen for preparation")
...
def _verify_parent_artifacts(protocol: dict[str, Any]) -> dict[str, Path]:
    ...
    for name, record in protocol["parent_artifacts"].items():
        ...
        if not path.is_file():
            raise FileNotFoundError(f"missing Prior v10 parent artifact: {path}")
        if file_sha256(path) != record["file_sha256"]:
            raise ValueError(f"Prior v10 parent artifact drift: {name}")
        paths[name] = path
    return paths
...
def _build(
    ...
    if any(overlap_report.values()):
        raise ValueError(f"Prior v10 overlap audit failed: {overlap_report}")
    ...
    report = {
        "status": "PASS_PRIOR_CANONICAL_SMOKE_V10_PACKAGES_READY",
        "schema_version": "clir-prior-canonical-smoke-pre-annotation-v10",
...
def _output_paths(root: Path) -> dict[str, Path]:
    filename = "prior_canonical_smoke.jsonl"
    return {
        "proposals": root / "proposals/prior_canonical_smoke_natural.jsonl",
...
def command_prepare(args: argparse.Namespace) -> None:
    ...
    if (
        ...
        raise FileExistsError("Prior v10 smoke output already exists; use verify-smoke")
...
def command_verify(args: argparse.Namespace) -> None:
    ...
    verification = {
        "schema_version": "clir-prior-canonical-smoke-independent-verification-v10",
        "status": (
            "PASS_PRIOR_CANONICAL_SMOKE_V10_RECOMPUTATION"
            if not mismatches
            else "FAIL_PRIOR_CANONICAL_SMOKE_V10_RECOMPUTATION"
        ),
...
def command_evaluate(args: argparse.Namespace) -> None:
    ...
    if verification.get("status") != "PASS_PRIOR_CANONICAL_SMOKE_V10_RECOMPUTATION":
        raise ValueError("Prior v10 package verification is not PASS")
